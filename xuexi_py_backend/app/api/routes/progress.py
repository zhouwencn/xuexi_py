...
from app.schemas.account import ProgressRead, ProgressWrite
...
def get_progress(
    ...
    if progress is None:
        return success_response(ProgressRead(course_id=course_id, state={}, version=0))
...
def save_progress(
    ...
    if len(json.dumps(payload.state, ensure_ascii=False)) > 256_000:
    ...
    if progress is None:
        progress = UserProgress(user_id=user.id, course_id=course_id, state=payload.state, version=1)
        session.add(progress)
    else:
        progress.state = payload.state
        progress.version += 1
    session.commit()
