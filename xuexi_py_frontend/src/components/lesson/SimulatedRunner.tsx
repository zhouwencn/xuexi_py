...
export function SimulatedRunner({ code }: { code: string; output?: string }) {
  ...
  return (
    <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm dark:border-white/10 dark:bg-[#0b1714]">
      ...
      <div className="flex min-h-20 items-start justify-between gap-4 border-t border-slate-200 bg-slate-50 p-4 dark:border-white/10 dark:bg-black/20"><div><div className="mb-1 text-[10px] font-bold uppercase tracking-[0.18em] text-slate-400">Output</div><pre className="whitespace-pre-wrap font-mono text-xs leading-5 text-slate-700 dark:text-slate-300">{result}</pre></div><button onClick={runCode} disabled={busy} className="btn-primary shrink-0 disabled:opacity-50"><Play size={14} fill="currentColor" /> {busy ? '运行中' : '运行'}</button></div>
    </div>
  )
}
